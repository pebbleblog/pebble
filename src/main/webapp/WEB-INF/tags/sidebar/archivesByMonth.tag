    ...
    <table width="99%" cellpadding="0" cellspacing="0">
      <c:forEach var="year" items="${archives.years}" varStatus="status">
        ...
        <td valign="top">
          ...
          <a href="${pebble:monthPermalink(permalinkProvider, month)}" title="Blog archive for ${month.date}"><fmt:formatDate value="${month.date}" pattern="MMMM"/></a> (<fmt:formatNumber value="${month.numberOfBlogEntries}" type="number" />)
          <br />
            </c:if>
          </c:forEach>
          <br />
