...
<div class="sidebarItem">
  ...
  <div class="sidebarItemBody">
    <c:forEach var="category" items="${categories}" begin="1" varStatus="status">
      <a href="${url:rewrite(category.permalink)}"><c:out value="${category.name}" escapeXml="true"/></a> (<fmt:formatNumber value="${category.numberOfBlogEntries}" type="number" />)
      <a href="${url:rewrite(category.permalink)}rss.xml" style="border: 0px;"><img src="common/images/feed-icon-10x10.png" alt="RSS feed" border="0" /></a>
      <br />
    </c:forEach>
  </div>
</div>
